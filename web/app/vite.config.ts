...
// Svelte 5's compiler strips type annotations from <script lang="ts"> but
// leaves optional-parameter `?` marks (e.g. `x?` from `x?: number`).
// Rollup's parser rejects that syntax. This plugin runs esbuild's TS strip
// on compiled .svelte output so the `?` is removed before Rollup sees it.
function svelteTypescriptCleanupPlugin() {
	return {
		name: 'svelte-ts-cleanup',
		enforce: 'post' as const,
		async transform(code: string, id: string) {
			if (!id.includes('.svelte') || !code.includes('?')) return;
			const result = await transformWithEsbuild(code, id + '.ts', {
				loader: 'ts',
				target: 'esnext'
			});
...
export default defineConfig({
	...
	plugins: [tailwindcss(), sveltekit(), svelteTypescriptCleanupPlugin(), devtoolsJson(), deltaBuildPlugin()],
	test: {
		projects: [
			{
