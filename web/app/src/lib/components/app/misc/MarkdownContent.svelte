<script lang="ts">
	...
	let loadedTheme = $state<string | null>(null);
	async function loadHighlightTheme(isDark: boolean) {
		if (!browser) return;
		const key = isDark ? 'dark' : 'light';
		if (loadedTheme === key) return;
		loadedTheme = key;
		...
		let style = document.querySelector<HTMLStyleElement>('style[data-highlight-theme]');
		if (!style) {
	...
	$effect(() => {
		loadHighlightTheme(mode.current === 'dark');
	});
