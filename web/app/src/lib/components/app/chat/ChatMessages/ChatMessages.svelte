<script lang="ts">
	...
	let displayMessages = $derived.by(() => {
		...
		for (const msg of allConversationMessages) nodeMap.set(msg.id, msg);

		return messages.map((message) => {
			const node = nodeMap.get(message.id);
			let siblingInfo: ReturnType<typeof getMessageSiblings> = null;
			if (node && node.parent !== null) {
				const parent = nodeMap.get(node.parent);
				if (parent && parent.children.length > 1) {
					siblingInfo = getMessageSiblings(allConversationMessages, message.id);
				}
			}
			return {
				message,
