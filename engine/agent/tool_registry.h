...
class ToolRegistry {
  public:
    ...
    // Maps the names small models invent (move_event, add_task, write_note...) onto a registered
    // tool. Returns `raw` unchanged when it is already registered or nothing matches. Task aliases
    // also set `type: task` on `arguments` when the model left it out.
    std::string resolve_alias(const std::string& raw, nlohmann::json& arguments) const;
