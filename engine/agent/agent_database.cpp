...
nlohmann::json AgentDatabase::get_note(const std::string& id) {
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        result = {{"id", col(0)},         {"title", col(1)},     {"content", col(2)},
                  {"folder", col(3)},     {"tags", col(4)},      {"pinned", sqlite3_column_int(stmt, 5) != 0},
...
bool AgentDatabase::update_note(const std::string& id, const nlohmann::json& data) {
    ...
    if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) != SQLITE_OK)
        return false;
    sqlite3_bind_text(stmt, 1, data.value("title", existing["title"].get<std::string>()).c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 2, data.value("content", existing["content"].get<std::string>()).c_str(), -1,
                      SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 3, data.value("folder", existing["folder"].get<std::string>()).c_str(), -1,
                      SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 4, data.value("tags", existing["tags"].get<std::string>()).c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_int(stmt, 5,
                     data.contains("pinned") ? (data["pinned"].get<bool>() ? 1 : 0)
...
std::vector<AgentDatabase::RpcNode> AgentDatabase::get_enabled_rpc_nodes() {
    ...
    sqlite3_stmt* stmt;
    const char* sql =
        "SELECT id, name, ip, port, enabled, created_at FROM worker_nodes WHERE enabled = 1 ORDER BY created_at DESC";
