...
static nlohmann::json filter_tools_by_config(const nlohmann::json& all_tools, bool use_calendar, bool use_notes) {
    if (use_calendar && use_notes)
        return all_tools;
    nlohmann::json filtered = nlohmann::json::array();
    std::set<std::string> calendar_tools = {"create_event", "list_events", "delete_event", "update_event",
                                            "get_current_time"};
...
//     for (auto& msg : messages) {
//         if (!cleaned.empty() && cleaned.back().value("role", "") == msg.value("role", "") && msg.value("role", "") !=
//         "system") {
...
//             std::string content = msg.value("content", "");
//             msg["content"] = "[Tool executed successfully. Result: " + content + "]\n\nNow respond to the user based
//             on this tool result."; msg.erase("tool_call_id"); msg.erase("name");
//         }
...
static std::string strip_tool_code_blocks(const std::string& text) {
    static const char* markers[] = {
        "TOOL_CALL",   "TOOL_CODE", "create_event(", "update_event(", "delete_event(",   "list_events(", "create_note(",
        "list_notes(", "get_note(", "update_note(",  "delete_note(",  "create_meeting(", "create_task(", nullptr};
    std::string out;
    size_t i = 0;
    while (i < text.size()) {
        ...
        for (int m = 0; markers[m]; m++) {
            if (block.find(markers[m]) != std::string::npos) {
                has_tool = true;
                break;
            }
        }
...
static bool user_wants_create(const nlohmann::json& messages) {
    ...
    static const char* create_words[] = {
        "create", "add",    "schedule", "book",    "remind",   "set up", "setup", "new ",      "plan ",      "set a",
        "set an", "set my", "make a",   "arrange", "organize", "log",    "jot",   "note down", "write down", nullptr};
...
static std::string normalize_tool_name(const std::string& raw) {
    ...
    if (n == "create_meeting" || n == "create_event" || n == "add_event" || n == "schedule_event" ||
        n == "create_appointment" || n == "set_meeting" || n == "create_reminder")
        return "create_event";
    if (n == "create_task" || n == "add_task" || n == "create_todo")
        return "create_event";
    if (n == "list_events" || n == "get_events" || n == "show_events" || n == "list_tasks")
        return "list_events";
    if (n == "update_event" || n == "move_event" || n == "reschedule_event" || n == "update_task")
        return "update_event";
    if (n == "delete_event" || n == "remove_event" || n == "delete_task")
        return "delete_event";
    if (n == "create_note" || n == "add_note" || n == "write_note")
        return "create_note";
    if (n == "list_notes" || n == "get_notes" || n == "show_notes")
        return "list_notes";
    if (n == "get_note" || n == "read_note")
        return "get_note";
    if (n == "update_note" || n == "edit_note")
        return "update_note";
    if (n == "delete_note" || n == "remove_note")
        return "delete_note";
    if (n == "get_current_time" || n == "current_time")
...
static bool extract_text_tool_call(const std::string& content, std::string& tool_name, nlohmann::json& args) {
    static const char* candidates[] = {
        "create_meeting", "create_event", "add_event",       "schedule_event",   "create_appointment", "create_task",
        "add_task",       "create_todo",  "create_reminder", "list_events",      "get_events",         "show_events",
        "list_tasks",     "update_event", "move_event",      "reschedule_event", "delete_event",       "remove_event",
...
void AgentLoop::set_response_format(const nlohmann::json& fmt) {
    response_format_ = fmt;
}
void AgentLoop::set_event_callback(EventCallback cb) {
...
nlohmann::json AgentLoop::build_request_body(const nlohmann::json& messages, nlohmann::json tools, bool stream) {
    ...
    if (!tools.empty()) {
        ...
        for (const auto& msg : messages) {
            if (msg.value("role", "") == "user") {
                ...
                if (text.length() > 150 || text.find("plan") != std::string::npos ||
                    text.find("step") != std::string::npos || text.find("complex") != std::string::npos ||
    ...
    if (!response_format_.empty())
        request_body["response_format"] = response_format_;
...
nlohmann::json AgentLoop::call_llm_stream(const nlohmann::json& messages, const nlohmann::json& tools,
    ...
    if (!ctx.saw_tool_calls && !ctx.suppress_forward)
...
AgentResponse AgentLoop::process(nlohmann::json messages, TokenCallback on_token) {
    ...
    try {
        ...
        return {false, "",    0, std::string("Failed to build system prompt: ") + e.what(), nlohmann::json::array(),
                0,     false, ""};
    }
    ...
    auto finish = [&](bool ok, const std::string& content, const std::string& error,
        ...
        return AgentResponse{ok,    content,  total_tool_calls, error, executed_tool_calls, forwarded_total,
                             false, reasoning};
    };
    ...
    for (int iteration = 0; iteration < max_iterations_; iteration++) {
        ...
        if (response.contains("error")) {
            auto& err = response["error"];
            std::string error_msg = err.is_string() ? err.get<std::string>()
        ...
        if (has_tool_calls) {
            ...
            for (auto& tool_call : message["tool_calls"]) {
                ...
                if (!registry.validate_arguments(tool_name, arguments, validation_error)) {
                    std::cerr << "[delta-agent] MODEL FORMATTING ERROR: Schema validation failed for " << tool_name
                              << ". Error: " << validation_error << ". Args: " << arguments.dump() << std::endl;
            ...
            for (size_t i = 0; i < futures.size(); ++i) {
                ...
                if (!result.success) {
                    std::cerr << "[delta-agent] TOOL EXECUTION ERROR: " << tool_name
        ...
        if (!has_tool_calls && !content.empty()) {
            ...
            if (extract_text_tool_call(content, parsed_name, parsed_args) && registry.has_tool(parsed_name)) {
                std::cerr << "[delta-agent] recovered text tool call: " << parsed_name << "(" << parsed_args.dump()
                          << ")" << std::endl;
                ...
                if (parsed_name == "create_event") {
                    ...
                    if (!parsed_args.contains("type"))
                        parsed_args["type"] = "event";
                }
        ...
        if (!action_retry_fired) {
            ...
            if (total_tool_calls == 0 && user_wants_create(messages)) {
                ...
                messages.push_back(
                    {{"role", "user"},
                     {"content", "Use the calendar tools to do that now. Never ask for a date -- pass it through as "
                                 "the user phrased it and let the tool resolve it. Do it now."}});
                tool_choice_ = "required";
                continue;
            }
        }
