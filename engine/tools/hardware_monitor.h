...
inline int estimate_model_layers(long long size_bytes) {
    if (size_bytes > 10LL * 1024 * 1024 * 1024)
        return 80;
    if (size_bytes > 5LL * 1024 * 1024 * 1024)
        return 48;
    if (size_bytes > 2LL * 1024 * 1024 * 1024)
        return 32;
    if (size_bytes > 1LL * 1024 * 1024 * 1024)
        return 28;
    return 24;
}
...
class HardwareMonitor {
  public:
     ...
     */
    ModelCompatibility check_model_compatibility(long long model_size_bytes, int n_layers, int requested_ctx) const;
