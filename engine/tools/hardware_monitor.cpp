...
#pragma pack(push, 1)
struct SMCVersion {
    UInt8 major, minor, build, reserved;
    UInt16 release;
};
struct SMCPLimit {
...
static float parse_smc_temp(const SMCKeyVal& kv) {
    ...
    if (strncmp(cc, "fpe2", 4) == 0 && kv.dataSize >= 2)
        return (float)(((d[0] << 8) | d[1]) / 4.0f);
    if (strncmp(cc, "ui8", 3) == 0)
        return (float)d[0];
    if (strncmp(cc, "ui16", 4) == 0 && kv.dataSize >= 2)
        return (float)((d[0] << 8) | d[1]);
    if (strncmp(cc, "si16", 4) == 0 && kv.dataSize >= 2)
        return (float)(int16_t)((d[0] << 8) | d[1]);
    if (kv.dataSize >= 2)
        return (float)(int8_t)d[0] + (float)d[1] / 256.0f;
    return 0.0f;
}
...
static float smc_cpu_temp_c() {
    ...
    static const char* keys[] = {"TC0D", "TC0P", "TC1D", "TC1P", "TC0E", "TC0F", "Tp0P", "Tp1P", "Tp2P", "Tp3P",
                                 "Tp4P", "Tp5P", "Tb0P", "Tb1P", "Tb2P", "Te0P", "Te1P", "Te2P", "Te3P", "Tm0P",
                                 "Tm1P", "Tm2P", "Ta0P", "Ta1P", "Th0P", "Th1P", "Th2P", nullptr};
    ...
    auto try_key = [&](const char* k) {
        ...
        if (IOConnectCallStructMethod(conn, SMC_KERNEL_INDEX, &in, sizeof(in), &out, &outSize) == KERN_SUCCESS) {
            ...
            if (t > 1.0f && t < 110.0f) {
                if (std::find(good.begin(), good.end(), k) == good.end())
                    good.push_back(k);
                if (t > best)
                    best = t;
            }
            ...
            if (services) {
                ...
                for (CFIndex i = 0; i < n; i++) {
                    ...
                    if (ev) {
                        ...
                        if (t > 110.0 && t < 11000.0)
                            t /= 100.0;
                        if (t > 0.0 && t < 110.0 && t > best)
                            best = (float)t;
                        CFRelease(ev);
                    }
                }
...
static float battery_temp_c() {
    ...
    float t = 0.0f;
    CFNumberRef num =
        (CFNumberRef)IORegistryEntryCreateCFProperty(bat, CFSTR("Temperature"), kCFAllocatorDefault, kNilOptions);
    if (num) {
        int v = 0;
        if (CFNumberGetValue(num, kCFNumberIntType, &v)) {
            float bt = (float)v / 100.0f;
            if (bt > 1.0f && bt < 110.0f)
                t = bt;
        }
        CFRelease(num);
    }
...
static int apple_gpu_utilization_pct() {
    ...
    while ((obj = IOIteratorNext(iter)) != 0) {
        CFDictionaryRef stats = (CFDictionaryRef)IORegistryEntryCreateCFProperty(obj, CFSTR("PerformanceStatistics"),
...
static float wmi_cpu_temp_c() {
    ...
    if (SUCCEEDED(CoCreateInstance(CLSID_WbemLocator, 0, CLSCTX_INPROC_SERVER, IID_IWbemLocator, (void**)&loc))) {
        IWbemServices* svc = nullptr;
        if (SUCCEEDED(loc->ConnectServer(_bstr_t(L"root\\WMI"), nullptr, nullptr, 0, 0, 0, 0, &svc))) {
            ...
            if (SUCCEEDED(svc->ExecQuery(_bstr_t(L"WQL"),
                ...
                if (cnt)
                    temp = (float)((sum / (double)cnt) / 10.0 - 273.15);
                en->Release();
            }
            svc->Release();
        }
...
void HardwareMonitor::probe_nvidia_nvml() {
    ...
    g_nvmlDeviceGetHandle =
        (nvmlDeviceGetHandleByIndex_v2_t)DELTA_DLSYM(nvml_lib_handle_, "nvmlDeviceGetHandleByIndex_v2");
    ...
    g_nvmlDeviceGetUtilization =
        (nvmlDeviceGetUtilizationRates_t)DELTA_DLSYM(nvml_lib_handle_, "nvmlDeviceGetUtilizationRates");
    ...
    if (g_nvmlDeviceGetCount)
        g_nvmlDeviceGetCount(&count);
...
void HardwareMonitor::probe_amd_rocm() {
    ...
    uint32_t count = 0;
    if (g_rsmi_num_devices)
        g_rsmi_num_devices(&count);
...
float HardwareMonitor::collect_cpu_util_pct() {
    ...
    i_time.LowPart = idle.dwLowDateTime;
    i_time.HighPart = idle.dwHighDateTime;
    k_time.LowPart = kernel.dwLowDateTime;
    k_time.HighPart = kernel.dwHighDateTime;
    u_time.LowPart = user.dwLowDateTime;
    u_time.HighPart = user.dwHighDateTime;
    ...
    prev_idle_time_ = i_time;
    prev_kernel_time_ = k_time;
    prev_user_time_ = u_time;
    ...
    uint64_t used =
        cpustats.cpu_ticks[CPU_STATE_USER] + cpustats.cpu_ticks[CPU_STATE_SYSTEM] + cpustats.cpu_ticks[CPU_STATE_NICE];
...
float HardwareMonitor::collect_system_power_w() {
    ...
    if (bat) {
        ...
        CFNumberRef a =
            (CFNumberRef)IORegistryEntryCreateCFProperty(bat, CFSTR("Amperage"), kCFAllocatorDefault, kNilOptions);
        CFNumberRef v =
            (CFNumberRef)IORegistryEntryCreateCFProperty(bat, CFSTR("Voltage"), kCFAllocatorDefault, kNilOptions);
        CFBooleanRef e = (CFBooleanRef)IORegistryEntryCreateCFProperty(bat, CFSTR("ExternalConnected"),
                                                                       kCFAllocatorDefault, kNilOptions);
        if (a) {
            CFNumberGetValue(a, kCFNumberIntType, &amp);
            CFRelease(a);
        }
        if (v) {
            CFNumberGetValue(v, kCFNumberIntType, &volt);
            CFRelease(v);
        }
        if (e) {
            ext = CFBooleanGetValue(e);
            CFRelease(e);
        }
        IOObjectRelease(bat);
        if (!ext && amp && volt)
    ...
    {
        float p = 0.0f;
        std::vector<GPUMetrics> g;
        collect_nvidia_metrics(g);
        for (auto& x : g)
            p += x.power_w;
        return p;
    }
...
void HardwareMonitor::collect_nvidia_metrics(std::vector<GPUMetrics>& out) {
    ...
    if (g_nvmlDeviceGetCount(&device_count) != NVML_SUCCESS)
        return;

    for (unsigned int i = 0; i < device_count; i++) {
        void* device = nullptr;
        if (g_nvmlDeviceGetHandle(i, &device) != NVML_SUCCESS)
            continue;
...
void HardwareMonitor::collect_apple_metrics(std::vector<GPUMetrics>& out) {
    ...
    if (sysctlbyname("machdep.cpu.brand_string", chip_name, &len, NULL, 0) == 0) {
        gpu.name = chip_name;
        if (std::string(chip_name).find("Apple") != std::string::npos)
    ...
    if (is_apple_silicon) {
        ...
        if (host_statistics64(mach_host_self(), HOST_VM_INFO64, (host_info64_t)&vmstat, &count) == KERN_SUCCESS) {
            ...
            unsigned long long used =
                (vmstat.active_count + vmstat.wire_count + vmstat.compressor_page_count) * page_size;
        ...
        if (acc) {
            CFNumberRef tot = (CFNumberRef)IORegistryEntryCreateCFProperty(acc, CFSTR("VRAM,totalMB"),
                                                                           kCFAllocatorDefault, kNilOptions);
            if (tot) {
                int mb = 0;
                CFNumberGetValue(tot, kCFNumberIntType, &mb);
                gpu.vram_total_gb = mb / 1024.0f;
                CFRelease(tot);
            }
            gpu.gpu_util_pct = apple_gpu_utilization_pct();
            IOObjectRelease(acc);
...
void HardwareMonitor::collect_amd_metrics(std::vector<GPUMetrics>& out) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        gpu.backend = "ROCm";
        char name[128] = {0};
        if (g_rsmi_dev_name && g_rsmi_dev_name(i, name, sizeof(name)) == 0)
            gpu.name = name;
        else
            gpu.name = "AMD GPU " + std::to_string(i);

        uint64_t total = 0, used = 0;
        if (g_rsmi_mem_total && g_rsmi_mem_total(i, 0, &total) == 0)
            gpu.vram_total_gb = (float)(total / (1024.0 * 1024.0 * 1024.0));
        if (g_rsmi_mem_used && g_rsmi_mem_used(i, 0, &used) == 0)
            gpu.vram_used_gb = (float)(used / (1024.0 * 1024.0 * 1024.0));

        uint32_t busy = 0;
        if (g_rsmi_busy && g_rsmi_busy(i, &busy) == 0)
            gpu.gpu_util_pct = (int)busy;

        int64_t temp = 0;
        if (g_rsmi_temp && g_rsmi_temp(i, 0, &temp) == 0)
            gpu.temp_c = (float)(temp / 1000.0);

        uint64_t power = 0;
        if (g_rsmi_power && g_rsmi_power(i, 0, &power) == 0)
            gpu.power_w = (float)(power / 1000000.0);
...
float HardwareMonitor::gpu_available_gb() const {
    ...
    if (rocm_available_ && g_rsmi_num_devices) {
        ...
        if (g_rsmi_num_devices(&c) == 0)
            for (uint32_t i = 0; i < c; i++) {
                uint64_t t = 0, u = 0;
                if (g_rsmi_mem_total && g_rsmi_mem_total(i, 0, &t) == 0 && g_rsmi_mem_used &&
                    g_rsmi_mem_used(i, 0, &u) == 0)
    ...
    if (metal_available_) {
        ...
        float reclaimable = (float)((vm.free_count + vm.inactive_count + vm.purgeable_count) * (uint64_t)ps /
...
OffloadPlan HardwareMonitor::plan_tiered_offload(long long model_size_bytes, int n_layers, int ctx_size) const {
    ...
    p.cpu_only = true;
    p.efficient = false;
    p.efficiency_warning = "Model too large for available memory. Needs " + std::to_string((int)model_gb) +
                           "GB but only " + std::to_string((int)gpu_free) + "GB GPU + " +
...
std::vector<std::string> HardwareMonitor::get_recommended_model_sizes() const {
    ...
    if (gpu_free > 0) {
        if (gpu_free >= 8.0f)
            recs.push_back("7B models (Q4_K_M, Q5_K_M)");
        if (gpu_free >= 16.0f)
            recs.push_back("13B models (Q4_K_M)");
        if (gpu_free >= 24.0f)
            recs.push_back("30B models (Q4_K_M)");
        if (gpu_free >= 40.0f)
            recs.push_back("65B models (Q4_K_M)");
    }

    if (recs.empty() && total_budget > 0) {
        if (total_budget >= 8.0f)
            recs.push_back("3B models (Q4_K_M) for CPU");
        if (total_budget >= 12.0f)
            recs.push_back("7B models (Q4_K_M) for CPU");
    }
...
std::vector<ModelEfficiency> HardwareMonitor::analyze_installed_models() const {
    ...
    std::sort(results.begin(), results.end(), [](const ModelEfficiency& a, const ModelEfficiency& b) {
        if (a.can_run != b.can_run)
            return a.can_run > b.can_run;
        if (a.efficient != b.efficient)
...
std::string HardwareMonitor::classify_process(const std::string& name) const {
    ...
    if (lower.find("chrome") != std::string::npos || lower.find("firefox") != std::string::npos ||
        lower.find("safari") != std::string::npos || lower.find("edge") != std::string::npos ||
        lower.find("brave") != std::string::npos) {
        return "browser";
    }
    if (lower.find("steam") != std::string::npos || lower.find("epic") != std::string::npos ||
        lower.find("unity") != std::string::npos || lower.find("unreal") != std::string::npos ||
        lower.find("gog") != std::string::npos || lower.find("origin") != std::string::npos) {
        return "game";
    }
    if (lower.find("code") != std::string::npos || lower.find("vscode") != std::string::npos ||
        lower.find("sublime") != std::string::npos || lower.find("atom") != std::string::npos ||
        lower.find("idea") != std::string::npos || lower.find("pycharm") != std::string::npos ||
        lower.find("webstorm") != std::string::npos) {
        return "editor";
    }
    if (lower.find("docker") != std::string::npos || lower.find("vmware") != std::string::npos ||
        lower.find("virtualbox") != std::string::npos || lower.find("parallels") != std::string::npos ||
        lower.find("utm") != std::string::npos) {
        return "virtualization";
    }
    if (lower.find("photoshop") != std::string::npos || lower.find("premiere") != std::string::npos ||
        lower.find("aftereffects") != std::string::npos || lower.find("finalcut") != std::string::npos ||
        lower.find("davinci") != std::string::npos || lower.find("figma") != std::string::npos ||
        lower.find("sketch") != std::string::npos || lower.find("blender") != std::string::npos) {
        return "creative";
    }
    if (lower.find("slack") != std::string::npos || lower.find("discord") != std::string::npos ||
        lower.find("teams") != std::string::npos || lower.find("zoom") != std::string::npos ||
        lower.find("telegram") != std::string::npos || lower.find("whatsapp") != std::string::npos) {
        return "communication";
    }
    if (lower.find("spotify") != std::string::npos || lower.find("apple music") != std::string::npos ||
        lower.find("youtube") != std::string::npos || lower.find("netflix") != std::string::npos) {
        return "media";
    }
    return "other";
}
...
std::vector<ResourceHog> HardwareMonitor::get_resource_hogs() const {
    ...
    for (const auto& pi : procs) {
        if (pi.pid == our_pid || pi.pid == 0 || pi.pid == 4)
            continue; // Skip self, System Idle, System
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        int parsed = sscanf(line, "%63s %d %f %f %ld %ld %15s %7s %15s %15s %1023[^\n]", user, &pid, &cpu, &mem_pct,
                            &vsz, &rss_kb, tty, stat, start, time_str, cmd);
        if (parsed < 11)
            continue;

        if (pid == our_pid || pid == 0 || pid == 1)
            continue;
        ...
        if (ram_gb < threshold_gb && cpu < 30.0f)
            continue;
        ...
        if (app_pos != std::string::npos)
            display_name = display_name.substr(0, app_pos);
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (pid == our_pid || pid == 0 || pid == 1 || pid == 2)
            continue;
...
ModelCompatibility HardwareMonitor::check_model_compatibility(long long model_size_bytes, int n_layers,
                                                              int requested_ctx) const {
    ...
    if (available_for_kv > 0 && n_layers > 0) {
        ...
        int max_ctx_possible = (int)((available_for_kv * 1024.0f * 1024.0f * 1024.0f) / (4.0f * (float)n_layers));
        ...
        if (max_ctx_possible >= 2048) {
            ...
            result.recommendation = "Reduce context to " + std::to_string(max_ctx_possible) + " tokens or less";
            ...
            result.recommendation = "Close resource-heavy applications or use a smaller model. "
                                    "Available memory: " +
                                    std::to_string((int)available_total) + "GB";
            result.suggested_context = 0;
        }
    } else {
