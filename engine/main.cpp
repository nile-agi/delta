...
#define DELTA_VERSION "dev"
    ...
    std::cout << "\n"
              << "╔══════════════════════════════════════════════════════════════╗\n"
              << "║                      DELTA CLI v" << DELTA_VERSION;
    // Pad to align the box
    size_t ver_len = std::string(DELTA_VERSION).size();
    for (size_t i = ver_len; i < 24; i++)
        std::cout << ' ';
    std::cout << "║\n"
...
QUICK START:
    delta pull qwen2.5:0.5b  # 1. Download a model
    delta --server            # 2. Start server + open Web UI

USAGE:
    delta --server              Start server with Web UI (recommended)
    delta                       Start interactive terminal mode
    delta [OPTIONS] [PROMPT]    One-shot query
    delta pull <model-name>     Download a model
    delta remove <model-name>   Remove a model

SERVER OPTIONS (delta --server):
    -m, --model <MODEL>         Specify model (auto-selects if omitted)
    --port <N>                  Server port (default: 8080)
    --np <N>                    Max parallel requests (default: 4)
    --c <N>                     Max context size (default: model native)
    --models-dir <DIR>          Scan directory for .gguf models
...
CLI OPTIONS:
    ...
    -g, --gpu-layers <N>        GPU layers (default: 0, use -1 for all)
...
EXAMPLES:
    delta pull qwen2.5:0.5b              # Download a model
    delta --server                        # Start with auto-selected model
    delta --server -m llama3.1:8b         # Start with specific model
    delta --server --port 9090            # Use custom port
    delta -m qwen2.5:0.5b "Hello world"  # One-shot query
    delta --list-models --available       # Browse available models
...
#define DELTA_VERSION "dev"
#endif
    std::cout << "Delta CLI v" << DELTA_VERSION << std::endl;
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-h" || arg == "--help") {
            ...
            if (!show_help && !show_version && !show_models && !interactive && !start_server && !check_updates &&
                !do_update && !config.multimodal) {
    ...
    if (start_server) {
        ...
        if (model_name.empty()) {
            // No -m specified: start server without a model, let user pick from UI
            if (models_dir.empty()) {
        ...
        std::string dev_candidates[] = {"build_macos/delta-server", "build_linux/delta-server",
                                        "build_windows/delta-server.exe", "build_android/delta-server",
        ...
        std::string install_locations[] = {
            "/usr/local/bin/delta-server", "/usr/bin/delta-server", "/opt/delta-cli/bin/delta-server",
        ...
        for (const auto& cand : server_candidates) {
            if (tools::FileOps::file_exists(cand)) {
                server_bin = cand;
                break;
            }
        }
        ...
        std::vector<std::string> public_candidates = {
            ...
            "/opt/homebrew/share/delta-cli/webui", "/usr/local/share/delta-cli/webui",
            ...
            tools::FileOps::join_path(exe_dir, "../public"), tools::FileOps::join_path(exe_dir, "../../public"),
        ...
        cmd << " --port " << server_port << " --parallel " << max_parallel;
        if (server_ctx > 0) {
            cmd << " -c " << server_ctx;
        }
        ...
        // Open browser in a background thread so the server runs in the foreground
        ...
        // Run delta-server in the foreground (inherits stdout/stderr for logs)
        ...
        if (!model_mgr.is_model_installed(model_name)) {
            ...
            if (!success) {
                ...
                if (!installed_models.empty()) {
                    UI::print_info("Found " + std::to_string(installed_models.size()) +
                                   " installed model(s) as fallback:");
                    ...
                    UI::print_info("  1. Check your internet connection and try: delta pull " +
